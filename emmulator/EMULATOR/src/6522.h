...
/*#
    ...
    *** THIS IMPLEMENTATION EMULATES THE ORIGINAL MOS 6522 ***
    *** For Western Design Center W65C22 compatibility, see comments marked "WDC" below ***
    *** To switch to W65C22 mode, replace the PCR macros with the WDC versions provided ***
    ...
    *************************************
    ...
    ## IMPORTANT: MOS 6522 vs WDC W65C22 DIFFERENCES
    
    *** MOS 6522 (ORIGINAL): ***
    - PCR bits Cx2,Cx1 = 00: Interrupt input (negative active edge)
    - PCR bits Cx2,Cx1 = 01: Interrupt input (positive active edge)  
    - PCR bits Cx2,Cx1 = 10: Independent interrupt input
    - PCR bits Cx2,Cx1 = 11: Manual output (fixed level)
    
    *** WDC W65C22: ***
    - PCR bits Cx2,Cx1 = 00: Interrupt input (negative active edge)
    - PCR bits Cx2,Cx1 = 01: Interrupt input (positive active edge)
    - PCR bits Cx2,Cx1 = 10: Independent interrupt input
    - PCR bits Cx2,Cx1 = 11: Pulse output (replaces MOS 6522's manual output)
    
    This implementation emulates the original MOS 6522.
    For Western Design Center W65C22 compatibility:
    1. UNCOMMENT the WDC_W65C22 define below
    2. The PCR macros will automatically switch to W65C22 mode
...
/* ============================================================================
   WDC W65C22 COMPATIBILITY SWITCH
   ============================================================================ */
/* Uncomment the following line to enable WDC W65C22 compatibility mode */
//#define WDC_W65C22
...
/* ============================================================================
   PCR TEST MACROS - MOS 6522 VERSION
   ============================================================================ */
#ifndef WDC_W65C22
/* MOS 6522 PCR test macros (MAME naming) */
...
#define M6522_PCR_CA2_AUTO_HS(c) ((c->pcr & 0x0e) == 0x08)
#define M6522_PCR_CA2_HS_OUTPUT(c) ((c->pcr & 0x0e) == 0x08)
/* FIXED: Use mask 0x0e to distinguish Pulse (0x0a) from Handshake (0x08) */
...
#define M6522_PCR_CB2_AUTO_HS(c) ((c->pcr & 0xe0) == 0x80)
#define M6522_PCR_CB2_HS_OUTPUT(c) ((c->pcr & 0xe0) == 0x80)
/* FIXED: Use mask 0xe0 to distinguish Pulse (0xa0) from Handshake (0x80) */
...
#else
/* ============================================================================
   PCR TEST MACROS - WDC W65C22 VERSION
   ============================================================================ */
/* WDC W65C22 PCR test macros */
/* WDC W65C22 PCR test macros - CORRECTED */
#define M6522_PCR_CA1_LOW_TO_HIGH(c) (c->pcr & 0x01)
#define M6522_PCR_CA1_HIGH_TO_LOW(c) (!(c->pcr & 0x01))
#define M6522_PCR_CB1_LOW_TO_HIGH(c) (c->pcr & 0x10)
#define M6522_PCR_CB1_HIGH_TO_LOW(c) (!(c->pcr & 0x10))

#define M6522_PCR_CA2_INPUT(c) (!M6522_PCR_CA2_PULSE_OUTPUT(c))
#define M6522_PCR_CA2_LOW_TO_HIGH(c) ((c->pcr & 0x0e) == 0x00)  /* 000 = neg edge input */
#define M6522_PCR_CA2_HIGH_TO_LOW(c) ((c->pcr & 0x0e) == 0x02)  /* 001 = pos edge input */
#define M6522_PCR_CA2_IND_IRQ(c) ((c->pcr & 0x0e) == 0x04)      /* 010 = independent int input */
#define M6522_PCR_CA2_OUTPUT(c) (M6522_PCR_CA2_PULSE_OUTPUT(c))
#define M6522_PCR_CA2_PULSE_OUTPUT(c) ((c->pcr & 0x0e) == 0x06) /* 011 = pulse output */

#define M6522_PCR_CB2_INPUT(c) (!M6522_PCR_CB2_PULSE_OUTPUT(c))
#define M6522_PCR_CB2_LOW_TO_HIGH(c) ((c->pcr & 0xe0) == 0x00)  /* 000 = neg edge input */
#define M6522_PCR_CB2_HIGH_TO_LOW(c) ((c->pcr & 0xe0) == 0x20)  /* 001 = pos edge input */
#define M6522_PCR_CB2_IND_IRQ(c) ((c->pcr & 0xe0) == 0x40)      /* 010 = independent int input */
#define M6522_PCR_CB2_OUTPUT(c) (M6522_PCR_CB2_PULSE_OUTPUT(c))
#define M6522_PCR_CB2_PULSE_OUTPUT(c) ((c->pcr & 0xe0) == 0x60) /* 011 = pulse output */

/* WDC doesn't have these modes */
#define M6522_PCR_CA2_AUTO_HS(c) (0)
#define M6522_PCR_CA2_HS_OUTPUT(c) (0)
#define M6522_PCR_CA2_FIX_OUTPUT(c) (0)
#define M6522_PCR_CA2_OUTPUT_LEVEL(c) (0)
#define M6522_PCR_CB2_AUTO_HS(c) (0)
#define M6522_PCR_CB2_HS_OUTPUT(c) (0)
#define M6522_PCR_CB2_FIX_OUTPUT(c) (0)
#define M6522_PCR_CB2_OUTPUT_LEVEL(c) (0)
#endif

/* ============================================================================ */

/* ACR test macros (MAME naming) - Same for both MOS and WDC */
...
#define M6522_SET_DATA(p, d)                        \
    {                                               \
        p = ((p & ~0xFF0000) | (((d)&0xFF) << 16)); \
    }
/* extract port A pins */
...
#define _M6522_PIP_SET(pip, offset, pos) \
    {                                    \
        pip |= (1 << (offset + pos));    \
    }
#define _M6522_PIP_CLR(pip, offset, pos) \
    {                                    \
        pip &= ~(1 << (offset + pos));   \
    }
/* reset an entire pipeline */
#define _M6522_PIP_RESET(pip, offset) \
    {                                 \
        pip &= ~(0xFF << offset);     \
    }
...
static inline void _m6522_read_port_pins(m6522_t *c, uint64_t pins)
{
    ...
    c->pa.c1_triggered =
        (c->pa.c1_in != new_ca1) &&
        ((new_ca1 && M6522_PCR_CA1_LOW_TO_HIGH(c)) ||
         (!new_ca1 && M6522_PCR_CA1_HIGH_TO_LOW(c)));

    c->pa.c2_triggered =
        (c->pa.c2_in != new_ca2) &&
        ((new_ca2 && M6522_PCR_CA2_LOW_TO_HIGH(c)) ||
         (!new_ca2 && M6522_PCR_CA2_HIGH_TO_LOW(c)));

    c->pb.c1_triggered =
        (c->pb.c1_in != new_cb1) &&
        ((new_cb1 && M6522_PCR_CB1_LOW_TO_HIGH(c)) ||
         (!new_cb1 && M6522_PCR_CB1_HIGH_TO_LOW(c)));

    c->pb.c2_triggered =
        (c->pb.c2_in != new_cb2) &&
        ((new_cb2 && M6522_PCR_CB2_LOW_TO_HIGH(c)) ||
         (!new_cb2 && M6522_PCR_CB2_HIGH_TO_LOW(c)));
...
/* FIXED: Added clear priority logic for CA2/CB2 output modes - WITH CORRECTED PRIORITY */
static inline uint64_t _m6522_write_port_pins(m6522_t *c, uint64_t pins)
{
    ...
    /* FIXED: CA2 output logic with CORRECT priority - FIXED OUTPUT BEFORE PULSE! */
    bool ca2_out;
    if (M6522_PCR_CA2_INPUT(c)) {
        // Input mode - reflect external pin state
        ca2_out = c->pa.c2_in;
    }
    else if (M6522_PCR_CA2_FIX_OUTPUT(c)) {
        // Fixed output mode - use stored level (MOS 6522 only)
        ca2_out = c->pa.c2_out;
    }
    else if (M6522_PCR_CA2_PULSE_OUTPUT(c)) {
        // Pulse output mode - use pulse pipeline
        ca2_out = (c->pa.c2_pulse_pip & 1) == 0;
    }
    else if (M6522_PCR_CA2_AUTO_HS(c) || M6522_PCR_CA2_HS_OUTPUT(c)) {
        // Handshake modes - use c2_out (set by CA1/RA access logic)
        ca2_out = c->pa.c2_out;
    }
    else if (M6522_PCR_CA2_IND_IRQ(c)) {
        // Independent interrupt input mode (WDC W65C22) - reflect input
        ca2_out = c->pa.c2_in;
    }
    else {
        // Fallback (shouldn't happen for valid PCR values)
        ca2_out = c->pa.c2_out;
    }
    ...
    /* FIXED: CB2 output logic with CORRECT priority - FIXED OUTPUT BEFORE PULSE! */
    bool cb2_out;
    if (M6522_PCR_CB2_INPUT(c)) {
        cb2_out = c->pb.c2_in;
    }
    else if (M6522_PCR_CB2_FIX_OUTPUT(c)) {
        cb2_out = c->pb.c2_out;
    }
    else if (M6522_PCR_CB2_PULSE_OUTPUT(c)) {
        cb2_out = (c->pb.c2_pulse_pip & 1) == 0;
    }
    else if (M6522_PCR_CB2_AUTO_HS(c) || M6522_PCR_CB2_HS_OUTPUT(c)) {
        cb2_out = c->pb.c2_out;
    }
    else if (M6522_PCR_CB2_IND_IRQ(c)) {
        // Independent interrupt input mode (WDC W65C22) - reflect input
        cb2_out = c->pb.c2_in;
    }
    else {
...
/* Force immediate update of all pin states */
static void _m6522_update_pins_now(m6522_t *c) {
    // Update PORTA pins
    c->pa.pins = (c->pa.inpr & ~c->pa.ddr) | (c->pa.outr & c->pa.ddr);
    
    // Update PORTB pins (with PB7 merging)
    c->pb.pins = _m6522_merge_pb7(c, (c->pb.inpr & ~c->pb.ddr) | (c->pb.outr & c->pb.ddr));
    
    // Update CA2/CB2 outputs based on PCR mode
    if (M6522_PCR_CA2_OUTPUT(c)) {
        if (M6522_PCR_CA2_FIX_OUTPUT(c)) {
            c->pa.c2_out = M6522_PCR_CA2_OUTPUT_LEVEL(c);
        }
        // Other modes (pulse, handshake) handled elsewhere
    }
...
static void _m6522_tick_sr(m6522_t* c) {
    if (M6522_ACR_SR_DISABLED(c)) {
        return;
    }
    if (c->sr_count == 8) {
        return;
    ...
    bool shift = false;
    /* phi2 modes */
    if (M6522_ACR_SI_O2_CONTROL(c) || M6522_ACR_SO_O2_CONTROL(c)) {
        shift = true;
    }
    /* T2 modes */
    else if (M6522_ACR_SI_T2_CONTROL(c) || M6522_ACR_SO_T2_CONTROL(c) || M6522_ACR_SO_T2_RATE(c)) {
        shift = c->t2.t_out;
    }
    /* external clock modes */
    else if (M6522_ACR_SI_EXT_CONTROL(c) || M6522_ACR_SO_EXT_CONTROL(c)) {
        shift = c->pb.c1_triggered && M6522_PCR_CB1_LOW_TO_HIGH(c);
    }

    if (shift) {
        /* shift-in */
        if (M6522_ACR_SI_T2_CONTROL(c) || M6522_ACR_SI_O2_CONTROL(c) || M6522_ACR_SI_EXT_CONTROL(c)) {
            c->sr = (c->sr << 1) | (c->pb.c2_in ? 1:0);
        }
        /* shift-out */
        else {
            c->pb.c2_out = (c->sr & 0x80) != 0;
            c->sr <<= 1;
        }
        c->sr_count++;
        if (c->sr_count == 8) {
            _m6522_set_intr(c, M6522_IRQ_SR);
        }
    }
}
...
static void _m6522_tick_pipeline(m6522_t *c)
{
    ...
    /* Clear stale pulse pipeline bits to prevent them from circulating */
    if (c->pa.c2_pulse_pip > 0xFF) {
        c->pa.c2_pulse_pip = 0;
    }
    if (c->pb.c2_pulse_pip > 0xFF) {
        c->pb.c2_pulse_pip = 0;
    }
}
...
static void _m6522_update_cab(m6522_t *c)
{
    if (c->pa.c1_triggered)
    {
        ...
        /* WDC W65C22: No auto-handshake mode, so only do this for MOS 6522 */
#ifndef WDC_W65C22
...
static uint64_t _m6522_tick(m6522_t *c, uint64_t pins)
{
    ...
    /* Update pins BEFORE writing them */
    c->pa.pins = (c->pa.inpr & ~c->pa.ddr) | (c->pa.outr & c->pa.ddr);
    c->pb.pins = _m6522_merge_pb7(c, (c->pb.inpr & ~c->pb.ddr) | (c->pb.outr & c->pb.ddr));
...
static uint8_t _m6522_read(m6522_t *c, uint8_t addr)
{
    ...
    case M6522_REG_SR:
        data = c->sr;
        _m6522_clear_intr(c, M6522_IRQ_SR);
        break;
...
static void _m6522_write(m6522_t *c, uint8_t addr, uint8_t data)
{
...
#endif
        if (M6522_PCR_CB2_PULSE_OUTPUT(c))
        {
            c->pb.c2_pulse_pip |= (1<<0);
        }
    ...
    case M6522_REG_PCR:
        /* FIXED: Clear pulse pipeline when not in pulse mode */
        c->pcr = data;
        
        /* MOS 6522: Fixed output mode sets CA2/CB2 level */
#ifndef WDC_W65C22
        if (M6522_PCR_CA2_FIX_OUTPUT(c))
        {
            c->pa.c2_out = M6522_PCR_CA2_OUTPUT_LEVEL(c);
            ...
            /* Clear pulse pipeline when leaving pulse mode */
            c->pa.c2_pulse_pip = 0;
        }
...
#else
        /* WDC W65C22: No fixed output mode, always clear pulse pipeline if not in pulse mode */
        if (!M6522_PCR_CA2_PULSE_OUTPUT(c)) {
            c->pa.c2_pulse_pip = 0;
        }
        if (!M6522_PCR_CB2_PULSE_OUTPUT(c)) {
...
#endif
        // UPDATE PIN STATES SINCE CA2/CB2 MIGHT HAVE CHANGED
        // This ensures bridge sees CA2 changes immediately
        break;
...
uint64_t m6522_tick(m6522_t *c, uint64_t pins)
{
    ...
    /* FIXME: move tick above read/write?
       The order of operations here is a fundamental design choice. The current
       implementation processes a CPU bus access first, and then ticks the
       internal state of the VIA. A real hardware chip would do these things
       in parallel. Changing the order is a complex change and could have
       unintended side effects, so it is left as is with this note that
       the timing may not be 100% cycle-accurate in edge cases.
    */
