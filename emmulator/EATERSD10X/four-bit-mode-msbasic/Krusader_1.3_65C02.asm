...
; Zero page storage	
IOBUF	=$0200		; I/O buffer for source code input and analysis
...
MISC2H	=$53	
K_TEMP1	=$54		; general purpose storage
K_TEMP2	=$55	
K_TEMP3	=$56	
K_TEMP4	=$57
LMNE	=K_TEMP3		; alias for compression and expansion routines
RMNE	=K_TEMP4
...
SHELL			; Loops forever
	...
	LDA #PROMPT
	JSR OUTCH	; prompt
	JSR K_OUTSP	; can drop this if desperate for 3 more bytes :-)
@KEY	JSR GETCH
	CMP #BS
...
@NEXT	LDY #$00
	...
	LDA KBD
	.else
	JSR MONRDKEY	; Check if a key was pressed
	BCC @NEXT	; If no key (Carry Clear), continue listing
	.endif
@RET	RTS
...
GETSZ			; SIZE = K_TEMP1,2 = lastlnL,H - MISCL,H + 1
	LDX #-$04
@LOOP	LDA CURLNH+1,X
...
;	CPY #$00
	BEQ LCLERR
	STY YSAV
@DOIT	JSR GOTOLN	; this leaves K_TEMP1 in Y and K_TEMP2 in X
	CPX #FAIL
	BEQ LCLERR
	LDA CURLNL
	STA MISC2L
	...
	BEQ @INC
	LDX K_TEMP4
	LDY K_TEMP3	; Validate the range arguments
	CPX K_TEMP2	; First compare high bytes
	BNE @CHK	; If TEMP4 != TEMP2, we just need to check carry
	CPY K_TEMP1	; Compare low bytes when needed
...
TOEND	
	LDA #$FF
	STA K_TEMP2	; makes illegal line number
;	JMP GOTOLN	; so CURLNL,H will be set to the end
; falls through

; ****************************************
	
GOTOLN			; go to line number given in K_TEMP1,2
			; sets CURLNL,H to the appropriate address
			; and leaves K_TEMP1 in Y and K_TEMP2 in X
...
@NXTLN			; is the current line number the same 
			; as specified in K_TEMP1,2?
			; Z set if equal
			; C set if K_TEMP1,2 >= LINEL,H 
	LDY K_TEMP1
	CPY LINEL
	BNE @NO
	LDX K_TEMP2
...
@SKIP	CMP #SP		; valid ASCII range is $20 to $5D
	...
	CPY #BS
	BEQ CMD_INPUT	; just do it all again
@NOBS	CPY #K_CR
	BNE LFAIL
...
INSSPC	
	LDA $01FF,X	; was previous character a space?
	CMP #SP
	BEQ @JUMP
...
@LOOP	LDA (PTCHTL),Y	; copy symbol to COMM
	...
	INY 
	LDA (PTCHTL),Y
	STA K_TEMP1	; save any offset value
...
CHKMOD	LDA CURMNE	; always > 0
	CMP ADDR_MIN,X	; mode index in X
...
@RET	RTS
@CONT	STA K_TEMP1	; store the value of the byte in TEMP1
	PLA
	CMP #PLUS
	BEQ @NONEG
	LDA K_TEMP1
...
@NONEG	LDA HADFRF
	BEQ @SKIP
	LDA K_TEMP1	; save the offset for use when patching
	STA (MISCL),Y	
@SKIP	;JMP ADD16X
; falls through
...
ADD16X			; Add a signed 8 bit number in K_TEMP1
			; to a 16 bit number in LVALL,H
			; preserve X (thanks leeeeee, www.6502.org/forum)
	LDA K_TEMP1	; signed 8 bit number
	BPL @CONT
...
GETMNE			; get mnemonic index for opcode in A
	...
	STA K_TEMP1	; will need it later
...
@SKIP	DEX
	...
	LSR
	LSR
	STA K_TEMP2	; save the high nibble
	LDA K_TEMP1
	AND #$0F
	CMP #$08
	BNE @NOTC1
	LDA K_TEMP2	; high nibble is our index
	RTS
@NOTC1	LDA K_TEMP1
	AND #$1F
	CMP #$10
...
@DOC3	LDA K_TEMP2
	...
	ADC #$10
	RTS
@NOTC3	LDX K_TEMP1	; does this code end in 10010?
	TXA
	AND #$1F
	CMP #$12
	BNE @1
...
ADDR_MIN			; Minimum legal value for MNE for each mode.
	.byte $00, $30, $25, $39
	.byte $20, $28, $34
	.byte $28, $28, $28
	.byte $20, $28, $28
...
CRLF			; Go to a new line.
	...
	.else
	JSR OUTCH
	LDA #K_LF		; "LF" - is this needed for the Apple 1?
	JMP OUTCH
	.endif
...
MONPROMPT          =     $5C             ;  Prompt character
                ...
                .segment "WOZMON"
WOZRESET        CLD                   ;  Clear decimal arithmetic mode
...
TOKRUSADER      JMP     SHELL         ;  Return to Krusader Shell
