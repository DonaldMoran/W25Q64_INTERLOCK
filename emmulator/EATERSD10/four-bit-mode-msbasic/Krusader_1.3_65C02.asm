...
; QUOTE	='''		; delimits a string	DON ALWAYS HAVE TO CHANGE THIS ONE
...
STRH = $43
KRU_SCRTCH = $44		; scratch location
...
MISC2H = $53
KRU_TEMP1 = $54		; general purpose storage
KRU_TEMP2 = $55
KRU_TEMP3 = $56
TEMP4 = $57
LMNE = KRU_TEMP3		; alias for compression and expansion routines
RMNE = TEMP4
...
; also the re-entry point
        ...
        LDA #PROMPT
        JSR OUTCH       ; prompt
        JSR KRU_OUTSP   ; can drop this if desperate for 3 more bytes :-)
    @KEY:
        JSR GETCH
        CMP #BS
    ...
    @OK:
        LDX #NUMCMD
    @NEXT:
        LDA CMDS - 1, X ; find the typed command
        CMP ARGS
        BEQ GOTCMD
        DEX
        BNE @NEXT
...
PANIC:
        ...
        LDA #$01
    @SKIP:
        STA (SRCSTL), Y ; Y is $00 from SHINIT
        RTS
...
; store them in KRU_TEMP1-4 as found
...
; INX
    ...
    @DONE:
        LDY YSAV
        RTS             ; m in KRU_TEMP1,2, n in KRU_TEMP3,4
...
; CPY #$00	; no args
        ...
        BCC @IN
    @ARGS:
        JSR GOTOLN      ; if no such line will insert at end
    @IN:
        JSR KRU_INPUT   ; Get one line
...
GETSZ:                  ; SIZE = KRU_TEMP1,2 = lastlnL,H - MISCL,H + 1
        LDX #$FC
    @LOOP:
        LDA CURLNH + 1, X
    ...
    @SKIP:
        LDX #$04
    @LOOP2:
        PLA             ; get CURLN and LINEN from the stack
        STA LINEL - 1, X
        DEX
        BNE @LOOP2
        RTS
...
; CPY #$00
        ...
        STY YSAV
    @DOIT:
        JSR GOTOLN      ; this leaves KRU_TEMP1 in Y and KRU_TEMP2 in X
        CPX #FAIL
        BEQ LCLERR
        LDA CURLNL
        STA MISC2L
...
; CMP #$01
        ...
        BEQ @INC
        LDX TEMP4
        LDY KRU_TEMP3   ; Validate the range arguments
        CPX KRU_TEMP2   ; First compare high bytes
        BNE @CHK        ; If TEMP4 != KRU_TEMP2, we just need to check carry
        CPY KRU_TEMP1   ; Compare low bytes when needed
    @CHK:
        BCC LCLERR      ; If carry clear, 2nd argument is too low
    @INC:
        INY             ; Now increment the second argument
        BNE @CONT
        INX
    @CONT:
        STX KRU_TEMP2
...
TOEND:
        LDA #$FF
        STA KRU_TEMP2   ; makes illegal line number
; JMP GOTOLN	; so CURLNL,H will be set to the end
; falls through

; ****************************************

GOTOLN:                 ; go to line number given in KRU_TEMP1,2
; sets CURLNL,H to the appropriate address
; and leaves KRU_TEMP1 in Y and KRU_TEMP2 in X
...
; as specified in KRU_TEMP1,2?
; Z set if equal
; C set if KRU_TEMP1,2 >= LINEL,H
        LDY KRU_TEMP1
        CPY LINEL
        BNE @NO
        LDX KRU_TEMP2
...
; Flag success with C flag
        ...
        .endif
    @SKIP:
        CMP #SP         ; valid ASCII range is $20 to $5D
        ...
        CPY #BS
        BEQ KRU_INPUT   ; just do it all again
    @NOBS:
        CPY #KRU_CR
        BNE LFAIL
        CPX #LABEL      ; KRU_CR at start of LABEL means a blank line
        BEQ DOBLNK
        LDA #EOL
        STA IOBUF, X
        BEQ GOTEOL
...
; RTS
    ...
    @FOUND:
        TAY             ; put mnemonic/directive code in Y
...
ISCMNT: STA MISCL
        ...
        STA IOBUF + $FFFF, X; A already holds $00
    @RET:
        RTS

ONEFLD:                 ; do one entry field
; A holds the end point for the field
        STA KRU_TEMP1   ; last position
    @NEXT:
        JSR NEXTCH      ; catches ESC, KRU_CR and BS
...
INSSPC:
        LDA IOBUF + $FFFF, X; was previous character a space?
        CMP #SP
        BEQ @JUMP
    @KRU_GET:
        JSR NEXTCH      ; handles BS, KRU_CR and ESC
        CMP #SP
        BNE @KRU_GET    ; only let SP through
    @JUMP:
        STA IOBUF, X    ; insert the space
        INX
        JMP OUTCH
...
ASSEM:                  ; Run an assembly
        ...
        JSR MYPRPC
    @NEXT:
        JSR DO1LN       ; line is in the buffer - parse it
; CPX #FAIL
        INX
        BEQ SHWERR
...
; Also display line if appropriate
    ...
    @UNKWN:
        LDY #UNKERR     ; else show unknown error
        BEQ @SKIP
        .endif
    @SHOW:
        CLC
...
; by the number of bytes in Y
        ...
        STY KRU_SCRTCH
        CLC
        ADC KRU_SCRTCH  ; move the current line pointer forward by 'Y' bytes
        STA CURLNL
        BCC INCLN
...
; BNE @CONT
    @CHABY:
        ...
        ADC #ADJABY
    @CONT:
        CPY #$23        ; STZ needs special handling
        BNE @DONE
        CPX #KRU_ABS
        BMI @DONE
        BEQ @SKIP
        ADC #$F1        ; carry is set
    @SKIP:
        ADC #$30 - 1    ; carry is set
    @DONE:
        JSR DOBYTE      ; we have the object code
        .if BRKAS2
        CMP #$00
        BNE @MKARG
        JSR DOBYTE
...
; word arg if X is greater than or equal to KRU_ABS
        CPX #KRU_ABS - 1
        BMI DOBYTE      ; X < #KRU_ABS
DOWORD: JSR DOBYTE
        LDA LVALH
DOBYTE: LDY #$00
        STA (CURPCL), Y
...
; LDA LABEL
        ...
        BEQ @STORE
    @SETPC:
        JSR ATOFR       ; output finishing module end address (+1)
...
; all are words
    ...
    @LOOP:
        LDA (PTCHTL), Y ; copy symbol to COMM
        STA COMM, Y
        INY
        CPY #SYMSZ
        BNE @LOOP
        ...
        INY
        LDA (PTCHTL), Y
        STA KRU_TEMP1   ; save any offset value
...
; BCS .MORE
    ...
    @RET:
        RTS
    @BRA:
        JSR ADDOFF      ; BRA instructions have a 1 byte offset argument only
        CPX #FAIL
        BEQ @ERR
    ...
    @ERR:
        ...
        JSR KRU_OUTSP
    @LOOP2:
        LDA (PTCHTL), Y ; Show symbol that failed
        JSR OUTCH
        INY
        CPY #SYMSZ
        BNE @LOOP2
...
; return mode in X
        ...
        BNE @NOTSP
    @EOL:
        CMP #$12        ; check exception first - JSR
        BEQ @RET
    ...
    @DOIMM:
        ...
        BEQ @RET
    @IMMOK:
        STX CURADM      ; handle immediate mode
; LDX #01	; skip the '#'
        DEX             ; X == IMM == 2
        JSR QTEVAL
        INX
...
; LDX #IMM
        RTS
    @NOTREL:
        LDX #0          ; check the more complicated modes
        LDA ARGS
...
CHKEXS:                 ; Current addressing mode is in X
        ...
        BNE OK
    @CONT:
        LDY #SPCNT      ; check special includes
    @LOOP:
        LDA SPINC1 - 1, Y; load mnemonic code
        CMP CURMNE
        BNE @NEXT
...
CHKMOD: LDA CURMNE      ; always > 0
        CMP KRU_MIN, X  ; mode index in X
        BCC BAD         ; mnemonic < KRU_MIN
...
; LDX #FAIL	; X is already $FF
    @RET:
        RTS
    @CONT:
        STA KRU_TEMP1   ; store the value of the byte in KRU_TEMP1
        PLA
        CMP #PLUS
        BEQ @NONEG
        LDA KRU_TEMP1
    ...
    @NONEG:
        LDA HADFRF
        BEQ @SKIP
        LDA KRU_TEMP1   ; save the offset for use when patching
        STA (MISCL), Y
    @SKIP:              ; JMP ADD16X
; falls through
...
ADD16X:                 ; Add a signed 8 bit number in KRU_TEMP1
; to a 16 bit number in LVALL,H
; preserve X (thanks leeeeee, www.6502.org/forum)
        LDA KRU_TEMP1   ; signed 8 bit number
        BPL @CONT
...
; by the required constant
        STX KRU_TEMP3   ; store start of the expression
        LDA ARGS, X
        CMP #LOBYTE
        BEQ @HASOP
        CMP #HIBYTE
        BNE @DOLBL
    @HASOP:
        STA FRFLAG      ; disables forward references when there
        INX             ; is a '<' or a '>' in the expression
        LDA ARGS, X
    @DOLBL:
        JSR CHKLBL      ; is there a label?
...
XCHKOP: LDY #$00
        ...
        STA LVALL
    @GETLO:
        STY LVALH       ; keep LVALL, and zero LVALH
    @STORE:
        LDA ARGS, X     ; copy rest of args to COMM
        STA COMM, Y
        BEQ @DOVAL
        CMP #SP
        BEQ @DOVAL
...
LB2VAL:                 ; label to be evaluated is in ARGS + X (X = 0 or 1)
    ...
    @NEXT:
        ...
        BNE @NEXT
    @STOP:
        LDA #SP         ; label is in COMM - ensure filled with spaces
    @LOOP:
        STA COMM, Y     ; Y still points to next byte to process
        INY
        CPY #LBLSZ
        BNE @LOOP
DOLVAL: LDA #<COMM      ; now get value for the label
    ...
    @SKIP:
        ...
        BNE @LOOP
    @CONT:
        STA MISCL       ; Reqd address is now in MISCL,H
...
STCONT: LDA #LABEL
    ...
    @DWNHI:
        DEC GSYMTH      ; carry bit clear - decrease high pointer
    @CONTG:
        STA GSYMTL
    ...
    @LSYM:
        LDA LSYMTH      ; Store symbol in local symbol table
        STA MISCH
...
;
; IMP (0)	ZPG (4) INZ (7) KRU_ABS (A) IND (D)
...
ASC2HX:                 ; convert ASCII code in A to a HEX digit
    ...
    @VALID:
        RTS
    @ERR:
        LDA #FAIL       ; this value can never be from a single digit,
...
DSMBL:
        ...
        STA COUNT
    @DSMBL2:
        JSR INSTDSP     ; Disassemble and display instr.
        JSR PCADJ
...
; ORA #$80	; * add indexing offset
    ...
    @ERR:
        LDY #$FC        ; Substitute $FC for invalid op,
    ...
    @PPADR2:
        ASL FORMAT      ; Test next print format bit
    ...
    @RELADR:
        JSR PCADJ3      ; PCL,H + DISPL + 1 to A,Y
        TAX
        INX
        BNE PRNTYX      ; *     +1 to X,Y
        INY
...
PRBL2:  JSR KRU_OUTSP   ; Output a blank
        DEX
...
; into the mnemonic table
        STA KRU_TEMP1   ; will need it later
    ...
    @SKIP:
        ...
        LSR
        LSR
        STA KRU_TEMP2   ; save the high nibble
        LDA KRU_TEMP1
        AND #$0F
        CMP #$08
        BNE @NOTC1
        LDA KRU_TEMP2   ; high nibble is our index
        RTS
    @NOTC1:
        LDA KRU_TEMP1
        AND #$1F
    ...
    @DOC3:
        ...
        RTS
    @NOTC3:
        LDX KRU_TEMP1   ; does this code end in 10010?
        TXA
        AND #$1F
        CMP #$12
        BNE @1
...
KRU_MIN:                ; Minimum legal value for MNE for each mode.
        .byte $00, $30, $25, $39
        .byte $20, $28, $34
        .byte $28, $28, $28
        .byte $20, $28, $28
...
MSGSZ = $21
MSG:    .byte "3.1 NESSEW NEK YB 20C56 REDASURK",KRU_CR
...
CRLF:                   ; Go to a new line.
        ...
        .else
        JSR OUTCH
        LDA #KRU_LF     ; "KRU_LF" - is this needed for the Apple 1?
        JMP OUTCH
        .endif
...
NXTPRNT:
        ...
        LDA #':'        ; Print colon
        JSR OUTCH
PRDATA: JSR KRU_OUTSP
        LDA (XAML, X)   ; Get data from address (X=0)
        JSR OUTHEX      ; Output it in hex format
XAMNEXT:
        STX MODEM       ; 0 -> MODE (XAM mode).
