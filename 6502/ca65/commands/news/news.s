; news.s - Fetch and display news headlines parsed by Pico.
; Usage: NEWS
;        NEWS <host> <port> <path>
;        NEWS SETDEFAULT <host> <port> <path>
...
start:
    ...
    ; --- Argument Parsing ---
    ; Find start of arguments after "RUN /BIN/NEWS.BIN"
    lda #<INPUT_BUFFER
    sta t_arg_ptr
    lda #>INPUT_BUFFER
    sta t_arg_ptr+1
    jsr skip_word ; "RUN"
    jsr skip_word ; "/BIN/NEWS.BIN"
    ...
    sta t_str_ptr1+1
    jsr strcmp
    bne @get_news       ; If not match, skip the jump
    jmp @do_setdefault  ; If match, jump to far label
...
@arg_loop:
    ...
    bne @next_char
    lda #0
    sta ARG_BUFF, x ; Replace space with null
@next_char:
    inx
    iny
    jmp @arg_loop
...
@do_setdefault:
    ...
    ; Pack host, port, path into ARG_BUFF separated by newlines
    ldx #0
    ldy #0
@pack_loop:
    lda (t_arg_ptr), y
...
str_cfg_path:   .asciiz "/etc/news.cfg"
msg_fetching:   .asciiz "Fetching headlines..."
msg_fail:       .asciiz "Failed."
msg_saved:      .asciiz "Default feed saved."
